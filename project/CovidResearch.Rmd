...
# Tested_positive = The variable that shows the total positive tested residents of the province Limburg
# Deceased = The variable that shows the total deceased that were tested positive of COVID
# Province = The chose province (Limburg)
...
# Estimate: The estimate is used to predict the value of the response variable. In this case the estimate is 2.9298884 deceased when there is a total positive tests at 0. By each time a new positive tests is reported, the deceased variable increases with -0.0001729 deceased.  
...
# Adjusted R-squared: The adjusted controls for each additional predictor added (to prevent overfitting), so it may not increase as you add more variables. 

# 6. Residuals Uitleggen
par(mfrow=c(2,2))
plot(lm(finalReg$Deceased~finalReg$Positive_tests))
# plot(lm(finalReg$Deceased~finalReg$Positive_tests+finalReg$Date_of_publication))
