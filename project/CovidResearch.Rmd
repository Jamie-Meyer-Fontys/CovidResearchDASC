...
## NIEUWE
# eerste regression
# aantal overleden per maand in de loop van 2020-2-28 tot en met 2022-3-28
# 
...
# summary(lm(Deceased ~ Date_of_publication, data = cov_general_data_clean))

x <- c(cov_general_data_clean_reg$month)
y <- c(cov_general_data_clean_reg$Deceased)

# summary(lm(y ~ x, data = cov_general_data_clean_reg))
...
# When total reported is 0, estimated deceased is 0.71123766 people. With every increase of total positives by 1, deceased goes up by 0.000238438. Thus, when there are 20000 positive reports then there are 0.71123766 + (0.000238438 * 20000) = 5.479 (5) deceased.
# 
# The standard error for Deceased is a distance of 0.000799811 deceased, which is the average distance that falls from the regression. For the Total positive reported, this standard error comes with a distance of 0.000007209 reports.
# 
# The null hypothesis from this regression (which is "There is a relation between Deceased and total reports from a COVID record") shows that the chance that the null hypothesis is true, is very low. In this case, it tells us that we cant say that the relationship between Deceased and Total_reports is very strong, nor that we are absolutely sure that a systematic relationship is present.
# tweede regression
