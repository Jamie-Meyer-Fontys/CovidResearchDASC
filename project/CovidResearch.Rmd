...
reg1 <- lm(Tested_positive ~ Tested_positive, data = cov_pt_sub)
summary(reg1)

plot(Tested_with_result ~ Tested_positive, data = cov_pt_sub, col="purple")
abline(reg1, col="red")
  ...
  stat_smooth(method="lm", color="yellow",  formula=y~x)

  ggplot(cov_pt_sub, aes(x=Security_region_name, y=Tested_with_result)) + 
  geom_col(aes(y = Tested_positive), fill="red") + geom_point() + geom_smooth(method="lm")
  
  ggplot(cov_pt_sub, aes(x=Tested_with_result, y=Tested_positive)) + 
  geom_point(color="purple") + geom_smooth(method="lm", color="red")
