...
```
```{r}
## Rename 'Fryslân' to 'Friesland' in cov_performed_tests
## TODO: Here rename 'Fryslân' to 'Friesland' for consistency sake.
...
total_pos_cov_tests_by_date <- aggregate(cov_performed_tests$Total_tested_positive, list(cov_performed_tests$Date_of_statistics), max)
...
ggplot(data = res_count_per_province_nl, aes(x = Province)) + 
  geom_bar(aes(y = Amount), stat = 'identity', fill = 'steelblue') +
  coord_flip() + 
...
# Tweede nieuwe regression

reg2 <- cov_general_data_clean %>% group_by(month=floor_date(Date_of_publication, "month")) %>%
   summarize(Deceased=sum(Deceased))

month <- c(reg2$month)
y <- c(reg2$Deceased)

plot(x,y,col = "blue",main = "Deceased per month (2020-2-28 | 2022-3-28)",
abline(lm(y~month)),cex = 1.3,pch = 16,xlab = "Month",ylab = "Deceased")
summary(lm(y ~ month, data = reg2))
...
# tweede regression
# aantal positieven per maand in de loop van 2020-2-28 tot en met 2022-3-28
cov_general_data_clean_reg <- cov_general_data_clean %>% group_by(month=floor_date(Date_of_publication, "month")) %>%
   summarize(Total_reported=sum(Total_reported))

summary(lm(Total_reported ~ Date_of_publication, data = cov_general_data_clean))

x <- c(cov_general_data_clean_reg$month)
y <- c(cov_general_data_clean_reg$Total_reported)

plot(x,y,col = "blue",main = "Total positive reports per month (2020-2-28 | 2022-3-28)",
...
plot(cov_general_data_clean$Total_reported,cov_general_data_clean$Deceased,col = "blue",main = "test",
abline(lm(Deceased~Total_reported, data=cov_general_data_clean)),cex = 1.3,pch = 16,xlab = "Total positive reports",ylab = "Deceased")
```