...
```{r}
  cov_pos_tests_per_province <- ggplot(data = cov_general_data, aes(Date_of_report, Total_reported, color = Province))
...
```{r}
  ...
  cov_pt <- ggplot(data = cov_pt_sub, mapping = aes(Security_region_name, Tested_with_result))
  cov_pt_plot <- cov_pt + geom_bar(stat="identity", fill = "steelblue") + coord_flip()
  cov_pt_plot
```