...
### Regression Goal
Find out the relation between the performed Covid-19 tests and the amount which came back positive.
...
Both variables come from the cov_performed_tests_clean tibble within the time frame from 2020-02-28 till 2022-03-29.
...
cov_performed_tests_clean %>%
  ggplot(aes(x = Tested_positive, y = Tested_with_result)) +
  geom_point() +
  geom_smooth(method = "lm", color = "red") +
  labs(title = "The relation between the performed and positive Covid-19 tests\n", x = "Positive tests", y = "Performed tests") 
```
...
In this regression the response variable is Tested_with_result, Tested_positive is the explanatory variable.
```{r}
summary(lm(Tested_with_result~Tested_positive, data = cov_performed_tests_clean))
```
...
```{r}
# add some plots showing the residuals
par(mfrow=c(2,2))
plot(lm(Tested_with_result~Tested_positive, data = cov_performed_tests_clean))
```
...
# Tested_positive = The variable that shows the total positive tested reports of the province Limburg
# Deceased = The variable that shows the total deceased that were tested positive of COVID-19
# Province = The chosen province (Limburg)
# Date of publication = Date of announcement to group the total deceased and total positive tests
...
# The coefficients
# Estimate: The estimate is used to predict the value of the response variable. In this case the estimate is 2.9298884 deceased when there is a total positive tests at 0. By each time a new positive tests is reported, the deceased variable increases with -0.0001729 deceased. At first, this does not make any sense, because it would be very strange if we already confirm one or more deaths when we don't have a positive test returned. In addition, the deceased count decreasing with -0.0001729 by every positive test also does not make much sense either. If I had to think of a reason why the deceased decreases, then my only guess would be that more residents in Limburg were getting vaccined per day during the lifespan of the dataset, but I hardly believe that this is the case.
