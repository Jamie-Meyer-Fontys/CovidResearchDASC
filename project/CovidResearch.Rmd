...
```{r}
  ...
  #TODO combine friesland and fryslan numbers
```
...
```{r}
  ...
  cov_pt <- ggplot(cov_pt_sub, aes(Tested_with_result, Security_region_name, color = Security_region_name))
  cov_pt <- ggplot(cov_pt_sub, aes(Tested_with_result, Security_region_name))
  cov_pt <- ggplot(cov_pt_sub, aes(Tested_with_result, Security_region_name))
  
  cov_pt_plot <- cov_pt + geom_bar(stat="identity", fill="steelblue") # use for rest
  cov_pt_plot <- cov_pt + geom_bar(stat = "identity") + coord_flip() # use for color per region
  cov_pt_plot <- cov_pt + geom_bar(stat = "identity")
  cov_pt_plot
```